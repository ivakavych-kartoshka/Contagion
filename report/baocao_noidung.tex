...
\begin{itemize}
  ...
  \item \textbf{Cách làm:} xây một khung đo gồm \emph{hai protocol độc lập}, chạy
        trên 5 model thuộc 5 nhà phát triển độc lập model, lấy số liệu ở cấp từng cạnh và cấp toàn mạng.
...
thuộc năm nhà phát triển độc lập khác nhau. Kết quả chính gồm bốn phát hiện, và đặc biệt có một
...
\begin{itemize}
  ...
  \item \textbf{5 model, 5 nhà phát triển độc lập:} Llama 3.3 70B (Meta), DeepSeek
        V3.2 (DeepSeek), Claude Sonnet 4.5 (Anthropic), Amazon Nova Pro (Amazon),
        qwen2.5:7b (Alibaba, chạy local, miễn phí).
...
Thứ tư, em chạy trên năm model thuộc năm nhà phát triển độc lập, ba topology, và lưu lại toàn
...
\noidung
\begin{itemize}
  \item \textbf{Tổng khối lượng đã chạy:}
        \begin{itemize}
          \item 46 thư mục kết quả (25 thư mục có cấu trúc chuẩn);
          \item \textbf{$\approx$ 9.070 lượt gọi model} (ước lượng cận dưới);
          \item \textbf{2.342 trial ``tự nhiên''};
          \item \textbf{4,3 giờ} thời gian máy ghi lại được (cận dưới).
        \end{itemize}
  \item \textbf{Độ phủ:} 5 model / 5 nhà phát triển · 2 protocol độc lập ·
        3 topology · 3 defense · 3 kiểu biến hình tấn công.
  \item \textbf{Cấu hình từng loại thí nghiệm:}
        ...
        \end{tabular}
        \end{center}
  \item \textbf{Cách đếm (nói rõ để không hiểu nhầm):} lượt gọi là \emph{ước lượng cận
        dưới}, tính từ cấu hình: (trial tự nhiên $\times$ số agent) $+$
        (trial/cạnh $\times$ số cạnh). Chưa tính các lần thử lại khi phải ép đầu ra bị
        nhiễm, và chưa tính các lần chạy hỏng hoặc bị hủy.
\end{itemize}

\loinoi
Trước khi vào kết quả, em xin báo cáo \emph{khối lượng} đã chạy, vì đây là câu hỏi thầy/cô
thường đặt ra và cũng là thứ quyết định độ tin cậy của các con số phía sau.

Tính đến nay, dự án đã tạo ra bốn mươi sáu thư mục kết quả, trong đó hai mươi lăm thư mục
có cấu trúc chuẩn dùng trực tiếp cho bài báo; phần còn lại là các thí nghiệm thăm dò giai
đoạn đầu. Tổng số lượt gọi model vào khoảng \emph{chín nghìn}, trên \emph{hai nghìn ba
trăm trial tự nhiên}. Thời gian máy ghi lại được là bốn phẩy ba giờ.

Em xin nói rõ hai điều về những con số này để không ai hiểu nhầm. Thứ nhất, con số chín
nghìn là \emph{ước lượng cận dưới}: em tính từ cấu hình của từng thí nghiệm --- số trial
tự nhiên nhân số agent, cộng số trial mỗi cạnh nhân số cạnh --- chứ không đếm từng lần
gọi. Cách đếm này bỏ sót các lần thử lại khi phải ép đầu ra thành nội dung bị nhiễm, và
bỏ sót các lần chạy hỏng hoặc bị hủy giữa chừng. Thứ hai, thời gian máy cũng là cận dưới,
vì hai họ script --- đường cong chiều sâu và mô hình có vòng --- không ghi lại thời gian
chạy.

Về cấu hình cụ thể. Mỗi ô chuẩn, tức mỗi model và mỗi topology, chạy bốn mươi lần tự
nhiên cộng ba mươi lần cho mỗi cạnh theo giao thức có kiểm soát. Thí nghiệm đường cong
chiều sâu chạy sáu mươi lần tự nhiên trên chuỗi dài, tới mười lăm agent với Llama và bảy
đến tám agent với hai model còn lại, vì chuỗi quá dài thì sẽ tắt hẳn và đường cong mất ý
nghĩa. Thí nghiệm mô hình có vòng chạy bốn mươi lần, mỗi lần năm vòng. Phần đo ảnh hưởng
lên công việc hợp lệ chạy hai mươi cặp sạch và bị tấn công cho mỗi cơ chế phòng thủ. Phần
độ nhạy chạy tám seed, mỗi seed hai mươi lần mỗi cạnh ở một temperature. Bảng biến hình
tấn công có sáu ô, mỗi ô ba mươi mẫu.

Điều em muốn nhấn mạnh là: con số chín nghìn tự nó không phải thành tích. Cái quan trọng
là \emph{mỗi ô đều có khoảng tin cậy và đều công bố độ lệch nhỏ nhất mà nó phát hiện
được}. Chạy thật nhiều mà không biết mình phát hiện được gì thì cũng không nói lên điều
gì.
...
\begin{itemize}
  ...
  \item Quy mô dữ liệu: 5 model thuộc 5 nhà phát triển độc lập, khoảng 45 thư mục kết quả, hàng chục nghìn
        lượt gọi model.
\end{itemize}
