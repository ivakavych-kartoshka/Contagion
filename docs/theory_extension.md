...
We say agent $v$ is **compromised** at a given interaction if its output, when treated as the injected data $x_{\text{inj}}$ for a downstream agent, satisfies a compromise criterion defined in terms of ASV/MR (formalized in Section 4). Let $C_a \in \{0,1\}$ be the indicator random variable that agent $a$ is compromised.

We define the **per-hop survival rate** (borrowing "survival" from epidemiological transmission-probability terminology, not to be confused with agent "shutdown") generically for a directed edge $u \to v$ as the conditional probability that a compromise at agent $u$ propagates to agent $v$:

$$
s_{u \to v} \;=\; \Pr\big(C_v = 1 \mid C_u = 1\big).
$$

**Chain indexing convention.** Throughout Sections 1–2, a *chain* has $k+1$ agents written $0, 1, \dots, k$ and $k$ hops, where hop $i$ ($i = 1, \dots, k$) is the directed edge $\text{agent}_{i-1} \to \text{agent}_i$. To match the chain product $\prod_{i=1}^{k} s_i$ used in Section 2 (and in `docs/formula_summary.md` §2), we write the per-hop survival rate of hop $i$ as

$$
s_i \;:=\; s_{\text{agent}_{i-1} \to \text{agent}_i} \;=\; \Pr\big(C_i = 1 \mid C_{i-1} = 1\big), \qquad i = 1, \dots, k.
$$

Equivalently, in the Liu-Gong notation, if agent $i-1$'s compromised output $\tilde{x}_{i-1}$ is passed downstream as the injected data for agent $i$, i.e. $x_{\text{inj}}^{(i)} = \tilde{x}_{i-1}$, then, under the threshold rule of Section 4.2,

$$
s_i = \Pr\Big(\text{ASV}_i \geq \tau_{\text{ASV}} \;\lor\; \text{MR}_i \geq \tau_{\text{MR}} \;\Big|\; C_{i-1} = 1\Big),
$$

where $\text{ASV}_i, \text{MR}_i$ are the scores of agent $i$'s output (the hop's receiver). $s_i$ is thus a **transmission probability** in the same sense as a secondary-attack rate in epidemiology: it is not the base attack success rate against a single isolated agent, but the _conditional_ rate given that the upstream agent is already compromised and is the source of the injected content.

#### 1.2 Estimation from controlled experiments

$s_i$ is estimated empirically, not derived analytically, since it depends on model behavior. Given a fixed pair of adjacent agent roles (the hop $i$ pair $\text{agent}_{i-1}, \text{agent}_i$) and a fixed attack strategy $A$:

1. Force agent $i-1$ into the compromised state ($C_{i-1} = 1$) via direct injection, so that its output $\tilde{x}_{i-1}$ is guaranteed to carry $s_{\text{inj}}$ or a downstream-effective encoding of it.
2. Feed $\tilde{x}_{i-1}$ as the untrusted input to agent $i$ and record whether $C_i = 1$ under the compromise criterion of Section 4.2.
...
$$
\hat{s}_i = \frac{1}{N}\sum_{r=1}^{N} \mathbb{1}\!\left[C_i^{(r)} = 1 \mid C_{i-1}^{(r)} = 1\right],
$$
...
$$
s_i = f\big(\text{role}_{i-1}, \text{role}_{i}, \text{topology-context}, \text{defense}_{i-1}, \text{defense}_{i}\big).
$$

Relevant dependencies to isolate experimentally:

- **Agent role**: an agent whose role is to summarize or relay raw external content (e.g., a web-browsing or retrieval agent) is likely to have higher inbound/outbound transmission than an agent whose role is narrow structured extraction (e.g., a JSON-schema validator), because instruction-following surface area differs by role.
- **Topology-context**: the same directed edge may exhibit different $s_i$ depending on in-degree at the receiver — an agent aggregating from multiple upstream sources may dilute or amplify injected instructions differently than one with a single upstream source. This motivates writing $s_{uv}$ (edge-indexed) rather than assuming a single scalar per chain position, as made explicit in Section 3.
- **Defenses**: paraphrasing, delimiting, instruction-hierarchy prompting, or detection-and-filtering defenses at the hop's receiver $\text{agent}_i$ act as multiplicative dampers, $s_i^{\text{defended}} = (1-\delta_{i})\, s_i^{\text{undefended}}$, where $\delta_{i}\in[0,1]$ is the defense's marginal interception rate at the receiver — itself measurable using the same trial procedure as Section 1.2 with the defense enabled.
...
We first assume the **first-order Markov property** on the compromise process along the chain: conditioned on $C_{i-1}$, the event $C_i$ is independent of $C_{i-2}, \dots, C_0$:

$$
\Pr(C_i \mid C_{i-1}, C_{i-2}, \dots, C_0) = \Pr(C_i \mid C_{i-1}), \qquad i = 1, \dots, k.
$$

This says that what matters for whether agent $i$ becomes compromised is _only_ the state handed to it directly by agent $i-1$ — not the full history of how the injection was originally worded several hops earlier. This is an assumption, not a derived fact, because in practice an injected instruction can be progressively rephrased, diluted, or reinforced as it passes through agents, which could induce longer-range dependence. We flag this explicitly as an empirically testable assumption (Section 2.5).

#### 2.3 Derivation of $P_{\text{end-to-end}}$

Under the Markov assumption, by the chain rule of probability and the tower property, for the chain of agents $0, 1, \dots, k$ with $C_0 = 1$:

$$
\Pr(C_k = 1 \mid C_0 = 1) = \sum_{c_1, \dots, c_{k-1}} \Pr(C_k=1 \mid C_{k-1}=c_{k-1}) \cdots \Pr(C_1=c_1 \mid C_0=1).
$$

Because compromise is only propagated forward along realized paths (an agent that fails to be compromised, $C_i = 0$, cannot pass on the injection — we assume no spontaneous re-compromise from non-injected content, i.e., $\Pr(C_i=1\mid C_{i-1}=0)=0$ for the pure-propagation model), the only nonzero path through the sum is the fully-compromised path $c_1 = c_2 = \cdots = c_{k-1} = 1$. Hence the sum collapses to a single product term:

$$
\boxed{P_{\text{end-to-end}} \;=\; \Pr(C_k=1 \mid C_0=1) \;=\; \prod_{i=1}^{k} s_i.}
$$

This is the direct multi-hop analogue of the Liu-Gong single-hop ASV: it is the probability that a chain of $k$ independent injection events, each with its own conditional success probability $s_i = \Pr(C_i=1 \mid C_{i-1}=1)$, all succeed in sequence.
...
This is the direct multi-neighbor generalization of the chain formula in Section 2: a chain is the special case where every internal node has a single outgoing edge ($|N(a)| = 1$, namely to $a{+}1$), for which the local reproduction number is $R_0^{(a)} = s_{a,\,a+1}$ (generic edge notation). In the hop-indexed notation of Section 1.1 this is the survival of the hop that *enters* agent $a{+}1$, i.e. $s_{a+1}$, so the branching process reduces to the deterministic-length chain of Section 2.
...
In the single-agent Liu-Gong setting, ASV and MR are computed once, on the single LLM output produced from $\tilde x$. In the multi-agent setting, we compute a **per-hop** ASV and MR pair at every hop traversed. Under the chain convention of Section 1.1 (hop $i$ = edge $\text{agent}_{i-1} \to \text{agent}_i$, receiver $\text{agent}_i$):

$$
\text{ASV}_i = \text{ASV}\big(y_i \mid x_{\text{inj}}^{(i)} = \tilde x_{i-1}\big), \qquad
\text{MR}_i = \text{MR}\big(y_i, y_i^{\text{direct}}\big),
$$

where $y_i$ is agent $i$'s actual output when fed agent $i-1$'s (possibly compromised) output as untrusted input, and $y_i^{\text{direct}}$ is the reference output agent $i$ would produce if directly and solely instructed to perform the injected task $s_{\text{inj}}$. (For a generic edge $u \to v$ the same expressions hold with receiver $v$ and source $u$.) This preserves the original semantics of ASV (task-performance under attack) and MR (behavioral hijack similarity) at each hop independently, rather than only at the network's final output.
...
$$
C_i = \mathbb{1}\!\left[\text{ASV}_i \geq \tau_{\text{ASV}} \;\;\text{or}\;\; \text{MR}_i \geq \tau_{\text{MR}}\right],
$$

using an "or" combination when either metric alone constitutes practically meaningful hijacking (e.g., a low-MR but high-ASV outcome, where the agent performs the injected task effectively but with different surface phrasing than the reference, should still count as compromised). The MR term is written uniformly as $\text{MR}_i \geq \tau_{\text{MR}}$; for tasks with an unambiguous, checkable injected-task ground truth, MR is an exact-match score and $\tau_{\text{MR}} = 1$ (equivalent to the stricter exact-match condition $\text{MR}_i = 1$). Thresholds $\tau_{\text{ASV}}, \tau_{\text{MR}}$ are calibrated per task family, following the same task-specific calibration practice used in the original Liu-Gong evaluation across their 7 tasks, rather than fixed universally.

#### 4.3 From ASV/MR distributions to $s_i$

Given a sample of $N$ trials at a fixed hop $i$, each producing a pair $(\text{ASV}_i^{(r)}, \text{MR}_i^{(r)})$, the per-hop survival rate defined in Section 1.1 is the empirical compromise rate under the thresholding rule of Section 4.2:

$$
\hat s_i = \frac{1}{N}\sum_{r=1}^N \mathbb{1}\!\left[\text{ASV}_i^{(r)} \geq \tau_{\text{ASV}} \;\lor\; \text{MR}_i^{(r)} \geq \tau_{\text{MR}}\right].
$$
