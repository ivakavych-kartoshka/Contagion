...
$|\prod_i \scite{i} - \mathrm{ASR}| / \mathrm{ASR}$.}
\label{tab:depth}
...
Table~\ref{tab:depth} gives the depth profile for Llama 3.3 70B: the magnitude of
the error rises with the number of hops the payload must survive, from $8\%$ at one
hop to $82\%$ at six. Repeating the measurement on DeepSeek V3.2 over a seven-hop
chain produces a different shape. Errors at depths $1\ldots7$ are $17$, $35$, $7$,
$30$, $14$, $10$ and $32\%$---a mean of $21\%$ with \emph{no} trend, fluctuating as
the product of seven edge estimates accumulates sampling noise rather than bias.

The two shapes are not a contradiction but a consequence of what
Section~\ref{sec:transport} measures directly. Whether the error compounds is
determined by whether the per-edge isolated rate transports at all. Where it
transports---as for DeepSeek, whose isolated product matches its measured ASR to
$2.2\%$---composing edge estimates introduces no systematic error, and the profile
is flat. Where it fails on one edge---as for Llama, whose middle edge is
under-estimated by $3.75\times$---that error enters every product containing the
edge and compounds with depth. The depth profile is therefore a cheap diagnostic:
one long chain, one run, and its slope indicates whether isolated per-hop rates
compose for that model at all.
...
in-chain survival. So the defensible statement is neither that isolated measurement
under-estimates spread nor that its error grows universally, but that
...
$ac + bd > 1$, and the measured value is $1.80$, so the conclusion tolerates
substantial error in the edge estimates. Second, this is a single three-agent
pattern at one temperature: we report it as a demonstration that recurrence makes
the criterion non-vacuous, not as a survey of cyclic structures.
