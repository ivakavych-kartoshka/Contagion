...
These structural effects are not specific to one model or one instance. Repeating
the star/tree comparison on Nova and DeepSeek and on a second Llama instance
($n = 10$) preserves the qualitative pattern wherever propagation is non-zero: on
DeepSeek the star reaches its aggregator more often than the tree
($\mathrm{ASR} = 0.75$ versus $0.475$), and on the second Llama instance the two
structures again separate reach from reproduction ($\mathrm{ASR} = 0.675$ star
versus $0.65$ tree at $n = 10$). Nova sits at the resistance floor
($\mathrm{ASR} = 0.00$ star, $0.125$ tree), where topology cannot move a signal that
is not there. The comparison therefore rests on three models and two instances, not
a single cell.
...
$\mathrm{ASR} = \prod_i \hat s_i = 0$ as uninformative. To confirm that the Llama
transport failure is a powered positive rather than an artefact of the coarse
$n = 40$ resolution, we re-ran its weak edge at $n = 200$ (200 natural runs, 200
controlled trials per edge): the minimum detectable effect falls to $0.11$ and the
composition deviation $\Delta = 0.615$ ($95\%$ interval $[0.54, 0.69]$,
$p = 0.0002$) rejects $H_0$ well above that threshold.
...
\emph{Transportability is a powered positive, not just an existence proof.} The
failure is established on the weak edge of one model; at $n = 200$ it is a powered
rejection ($\Delta = 0.615$, $p = 0.0002$, $\mathrm{MDE} = 0.11$;
Section~\ref{sec:validation}), so isolation \emph{does} fail there by $3.75\times$.
How \emph{often} it fails across edges and models remains uncharacterised, and the
other models we tested transport within resolution.
...
four to fifteen agents plus star and tree cells; the star/tree comparison is now
replicated across three models (Llama, Nova, DeepSeek) and two Llama instances
($n = 7$ and $n = 10$), so it is no longer single-model or single-instance, but
Section~\ref{sec:cyclic} is still a single recurrent instance. Tens of agents,
mesh/debate topologies, re-injection variants, and further defence families
(detection, hop isolation, structured queries~\cite{StrUQ2025}) are not varied
experimentally.
...
injected payload---no real secret or harmful capability is elicited---and every
model is queried through standard provider APIs. The attack forms we report
(literal, spaced, split-word) are already documented for the single-agent setting;
we add no novel exploit, and release code and data to help defenders measure and
mitigate multi-hop contamination.
...
Appendix~\ref{app:cluster}, and the threshold sweep of Appendix~\ref{app:tau})
reproduce our tables and figures without any model call; \textbf{(ii)} a free
local-model tier (\texttt{qwen2.5:7b}) for the depth-profile and cyclic cells; and
\textbf{(iii)} the hosted frontier cells, which need provider API access. A
\texttt{REPRODUCE} manifest maps each table to its command, cost, and caveats.
...
%%% 8 pages, which it does: Sections 1-8 end on page 8). All appendix numbers are
%%% produced offline by scripts/appendix_stats.py from stored results (0 API).
\appendix
