...
role-conditioned per-hop survival for five models from four families, and one of our
...
and $+0.200$. The star and the tree are thus independent checks in which the
independence assumption approximately holds, while the chain cells are where it
fails---consistent with the view that what breaks independence is not the network
but the repeated re-packaging of the payload along it.
...
manager broadcasting to two workers that report back.

With these per-hop survivals, the eigenvalue equation for
$M = \left(\begin{smallmatrix}0&a&b\\ c&0&0\\ d&0&0\end{smallmatrix}\right)$
gives $\lambda^2 = ac + bd$, hence
$\rho = \sqrt{a\,c + b\,d} = \sqrt{1.000{\times}0.867 + 1.000{\times}0.933} =
1.342$. The \emph{same} survivals read as a feed-forward network
($m\!\to\!w_1$, $m\!\to\!w_2$ only) give $\rho = 0$ by triangularity. The threshold
verdict therefore flips from vacuously subcritical to supercritical on the presence
of two feedback edges, with every $s$ held fixed (Table~\ref{tab:cyclic}).

\begin{table}[t]
\small
\caption{The same agents and the same per-hop survivals, read two ways. Feedback
turns a vacuous criterion into a binding one.}
\label{tab:cyclic}
\begin{tabular}{lccc}
\toprule
\textit{reading} & \textit{edges} & \textit{$\rho(M)$} & \textit{verdict} \\
\midrule
feed-forward   & 2 & 0.000 & subcritical (vacuous) \\
with feedback  & 4 & \textbf{1.342} & \textbf{supercritical} \\
\bottomrule
\end{tabular}
\end{table}

The empirical consequence matches the prediction. The feed-forward reading of the
same agents terminates at the workers (both compromised in $1.000$ of runs); that
is the whole of its propagation. In the recurrent run the manager---the agent that
sent the payload---is re-compromised by its own workers' reports in
$0.725$ $[0.572, 0.839]$ of runs, and the payload does not die out: at round five
the manager is still compromised in $0.700$ of runs and some agent in
$1.000$ $[0.912, 1.000]$, with the workers near saturation. The system settles into
an endemic state rather than decaying, which is what $\rho > 1$ predicts.
...
unreliable; the verdict survives because it is not marginal---it needs only
$ac + bd > 1$ and the measured value is $1.80$, so it tolerates substantial error in
the edge estimates. Second, this is a single three-agent pattern at one temperature:
a demonstration that recurrence makes the criterion non-vacuous, not a survey of
cyclic structures.
